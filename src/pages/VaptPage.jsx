...
import { Upload, X, CheckCircle2, ArrowUpRight, FileText, Send, Search, ChevronDown } from 'lucide-react';
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-ink-700">
          <div>
            <div className="flex items-center gap-3 mb-0.5">
              <p className="text-white font-bold text-sm">{request.id}</p>
              <span className={cn('tag text-[10px]', st.color)}>{st.label}</span>
            </div>
            <p className="text-slate-500 text-xs">{request.serviceType} · {request.companyName}</p>
          </div>
...
export default function VaptPage() {
  ...
  return (
    <div className="bg-ink-950 text-white min-h-screen">
      {/* Hero */}
      <section className="pt-32 pb-16 border-b border-ink-700">
        <div className="max-w-6xl mx-auto px-6 lg:px-8">
          <p className="text-teal-400 text-xs font-mono uppercase tracking-widest mb-6">Client Portal</p>
          <h1 className="text-5xl md:text-6xl font-black tracking-tighter leading-none mb-4 text-white">
            Client Security Portal
          </h1>
          <p className="text-slate-400 text-base">Manage your vulnerability assessments and security audits.</p>
        </div>
      </section>
        ...
        <div className="grid lg:grid-cols-5 gap-12">
          ...
          <div className="lg:col-span-3">
            {step === 3 ? (
              <div className="card p-10 text-center">
                ...
                <p className="text-slate-400 text-sm mb-2">Your reference ID:</p>
                <p className="font-mono text-teal-400 text-xl font-bold mb-6">{submittedId}</p>
                <p className="text-slate-500 text-sm mb-8">Save this ID to track your request below. Our team will respond within 24 hours.</p>
              ...
              <div className="card">
                ...
                <div className="flex border-b border-ink-700">
                  {['Contact & Scope', 'Documents & Submit'].map((label, i) => (
                    <div key={label} className={cn(
                      'flex-1 px-6 py-4 text-xs font-semibold border-r border-ink-700 last:border-r-0 transition-colors',
                      step === i + 1 ? 'text-teal-400 bg-teal-500/5' : 'text-slate-600'
                    )}>
                      <span className={cn('inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] mr-2 font-black',
                        step === i + 1 ? 'bg-teal-500 text-ink-950' : 'bg-ink-700 text-slate-500'
                      )}>{i + 1}</span>
                      {label}
                    </div>
                  ))}
                ...
                {step === 1 && (
                  <form onSubmit={handleNext} className="p-8 space-y-5">
                    <h2 className="text-white font-bold text-lg mb-2">Submit VAPT Scope</h2>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="label">Company Name *</label>
                        <input {...register('companyName', { required: true })} placeholder="Acme Corp" className="input-field" />
                        {errors.companyName && <p className="text-red-400 text-xs mt-1">Required</p>}
                      </div>
                      <div>
                        <label className="label">Contact Name *</label>
                        <input {...register('contactName', { required: true })} placeholder="John Doe" className="input-field" />
                        {errors.contactName && <p className="text-red-400 text-xs mt-1">Required</p>}
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                    ...
                    <div>
                      ...
                      {errors.serviceType && <p className="text-red-400 text-xs mt-1">Required</p>}
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      {errors.scope && <p className="text-red-400 text-xs mt-1">Required</p>}
                    </div>
                    <button type="submit" className="btn-teal w-full justify-center text-sm py-3">
                ...
                {step === 2 && (
                  <div className="p-8 space-y-6">
                    <h2 className="text-white font-bold text-lg mb-2">Upload Supporting Documents</h2>
                    <div
                      ...
                      className={cn(
                        ...
                        dragging ? 'border-teal-400 bg-teal-500/5' : 'border-ink-500 hover:border-ink-400 bg-ink-900'
                      )}
                    >
                      <Upload size={24} className="text-slate-500 mx-auto mb-3" />
                      <p className="text-slate-300 text-sm font-medium">Drag and drop files here</p>
                      <p className="text-slate-600 text-xs mt-1">Network diagrams, API documentation, or NDAs (PDF, DOCX)</p>
                    ...
                    {files.length > 0 && (
                      <ul className="space-y-2">
                        {files.map((f, i) => (
                          <li key={i} className="flex items-center justify-between card-subtle px-4 py-3 rounded-lg">
                            <div className="flex items-center gap-2">
                              <FileText size={13} className="text-teal-400 flex-shrink-0" />
                              <span className="text-slate-300 text-sm truncate max-w-xs">{f.name}</span>
                            </div>
                    ...
                    <div className="border-t border-ink-700 pt-5 space-y-3">
                      <h3 className="text-white font-semibold text-sm">Review Your Submission</h3>
                      <div className="card-subtle rounded-xl p-4 text-xs text-slate-400 space-y-1.5">
                        <p><span className="text-slate-300">Company:</span> {formData.companyName}</p>
                        <p><span className="text-slate-300">Service:</span> {formData.serviceType}</p>
                        <p><span className="text-slate-300">Environment:</span> {formData.targetEnvironment}</p>
                        <p><span className="text-slate-300">Files:</span> {files.length} attached</p>
                      </div>
                    </div>
                    <div className="flex gap-3">
                      <button onClick={() => setStep(1)} className="btn-outline flex-1 text-sm py-3 justify-center">Back</button>
                      <button onClick={handleSubmitFinal} className="btn-teal flex-1 text-sm py-3 justify-center">
                        Submit Request
                      </button>
                    </div>
                  </div>
          ...
          <div className="lg:col-span-2">
            <div className="card p-6">
              <h3 className="text-white font-bold text-sm mb-1">Track a Request</h3>
              <p className="text-slate-500 text-xs mb-5">Enter your reference ID to check status and view messages.</p>
              <div className="flex gap-2 mb-4">
                <input
                  value={lookupId}
                  onChange={e => { setLookupId(e.target.value); setLookupError(''); }}
                  onKeyDown={e => e.key === 'Enter' && handleLookup()}
                  placeholder="VAPT-2024-001"
                  className="input-field flex-1 text-sm py-2.5 font-mono"
                />
                <button onClick={handleLookup} className="btn-teal text-sm px-4 py-2.5">
                  <Search size={14} />
                </button>
              </div>
              {lookupError && <p className="text-red-400 text-xs mb-4">{lookupError}</p>}
              {lookupResult && st && (
                <div className="space-y-3">
                  <div className="card-subtle rounded-xl p-4">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-white font-semibold text-sm">{lookupResult.companyName}</p>
                      <span className={cn('tag text-[10px]', st.color)}>{st.label}</span>
                    </div>
                    <p className="text-slate-500 text-xs font-mono mb-1">{lookupResult.id}</p>
                    <p className="text-slate-400 text-xs">{lookupResult.serviceType}</p>
                  </div>
                  <button
                    onClick={() => setThreadRequest(lookupResult)}
            ...
            <div className="mt-4 card p-6 space-y-4">
              <h3 className="text-white font-bold text-sm">What to expect</h3>
              {[
                ...
                <div key={s} className="flex items-start gap-3">
                  <span className="w-5 h-5 rounded-full bg-teal-500/10 border border-teal-500/20 flex items-center justify-center text-teal-400 text-[10px] font-black flex-shrink-0 mt-0.5">{i + 1}</span>
