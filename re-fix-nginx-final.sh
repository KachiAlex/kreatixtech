...
sudo sed -i 's/listen 80 default_server;/listen 80;/' /etc/nginx/sites-available/familytree || true
sudo sed -i 's/listen 80 default_server;/listen 80;/' /etc/nginx/sites-available/default || true

# Update kreatixtech config
...
echo "FINAL VERIFICATION - SITE LOGS:"
