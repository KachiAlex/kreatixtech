...
export async function authMiddleware(request: Request, env: any): Promise<{ user: JwtPayload | null; error?: Response }> {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    return { user: null, error: new Response(JSON.stringify({ error: 'No token provided' }), { status: 401, headers: { 'Content-Type': 'application/json' } }) };
  }
  ...
  if (payload.type !== 'access') {
    return { user: null, error: new Response(JSON.stringify({ error: 'Invalid token type' }), { status: 401, headers: { 'Content-Type': 'application/json' } }) };
  }
...
export async function requireAdmin(request: Request, env: any): Promise<{ user: JwtPayload | null; error?: Response }> {
  ...
  if (user!.role !== 'admin') {
    return { user: null, error: new Response(JSON.stringify({ error: 'Admin access required' }), { status: 403, headers: { 'Content-Type': 'application/json' } }) };
  }
  return { user };
}
